-- DML: Enhanced Seed Data for Energy as a Service Platform
...
-- Insert sample energy usage data (last 7 days for performance)
...
SELECT 
    ...
    CASE 
        WHEN b.building_type = 'office' THEN 
            ...
            CASE WHEN EXTRACT(hour FROM ts) BETWEEN 8 AND 18 THEN 1.2 ELSE 0.6 END
        WHEN b.building_type = 'retail' THEN 
            b.baseline_consumption / 720.0 * (0.9 + 0.2 * random()) *
            CASE WHEN EXTRACT(hour FROM ts) BETWEEN 10 AND 22 THEN 1.1 ELSE 0.7 END
        WHEN b.building_type = 'warehouse' THEN 
            b.baseline_consumption / 720.0 * (0.85 + 0.3 * random()) *
            CASE WHEN EXTRACT(hour FROM ts) BETWEEN 6 AND 18 THEN 1.0 ELSE 0.8 END
        WHEN b.building_type = 'manufacturing' THEN 
            b.baseline_consumption / 720.0 * (0.9 + 0.2 * random()) *
            CASE WHEN EXTRACT(hour FROM ts) BETWEEN 6 AND 22 THEN 1.0 ELSE 0.9 END
    ...
    65 + 20 * sin(2 * pi() * EXTRACT(doy FROM ts) / 365.0) + 10 * random() - 5 as weather_temp_f,
    -- Generate occupancy data
    CASE 
        WHEN b.building_type = 'office' THEN 
            CASE WHEN EXTRACT(hour FROM ts) BETWEEN 8 AND 18 THEN 75 + 20 * random() ELSE 5 + 10 * random() END
        WHEN b.building_type = 'retail' THEN 
            CASE WHEN EXTRACT(hour FROM ts) BETWEEN 10 AND 22 THEN 40 + 30 * random() ELSE 5 + 10 * random() END
        ELSE 80 + 15 * random()
...
LIMIT 10000; -- Limit to prevent excessive data generation
